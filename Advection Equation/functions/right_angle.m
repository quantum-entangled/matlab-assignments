function Uold = right_angle(x, V, Courant, dh, Uinit, xStartStep, Lstep, xLeft, xRight, TextSize, t_fin)
    ...
    while t(i) <= t_fin
        ...
        if i == plot_time
            ...
            grid on;
            title(['Right Angle, ', 'Time = ', num2str(t(i)), ', dt = ', num2str(dt)], 'FontSize', TextSize);
            xlabel('X');
            ylabel('U');
            xlim([xLeft, xRight]);
            ylim([0, Uinit*1.5]);
