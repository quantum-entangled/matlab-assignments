function Uold = lax(x, V, Courant, dh, Uinit, xStartStep, Lstep, xLeft, xRight, TextSize, t_fin)
    ...
    while t(i) <= t_fin
        ...
        % Схема Лакса:
        Unew(2:end-1) = ((Uold(3:end) + Uold(1:end-2)) / 2) - V * dt / (2 * dh) * (Uold(3:end) - Uold(1:end-2));
        
        % Периодические граничные условия:
        Unew(1) = ((Uold(2) + Uold(end)) / 2) - V * dt / (2 * dh) * (Uold(2) - Uold(end));
        Unew(end) = ((Uold(1) + Uold(end-1)) / 2) - V * dt / (2 * dh) * (Uold(1) - Uold(end-1));
        ...
        if i == plot_time
            ...
            grid on;
            title(['Lax, ', 'Time = ', num2str(t(i+1)), ', dt = ', num2str(dt)], 'FontSize', TextSize);
            xlabel('X');
            ylabel('U');
            xlim([xLeft, xRight]);
            ylim([0, Uinit*1.5]);
